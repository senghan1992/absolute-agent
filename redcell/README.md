...
### 작업형 목표 (`GoalAgent`) — "합격자 목록을 파일로 뽑아줘" 도 엔진이 직접 한다

취약점 스캔이 아니라 **작업**(읽기·추적·다운로드·정리 저장)이 목표일 때를 위한 전용 에이전트다.
prime-agent(pi) 가 하던 것을 redcell 엔진이 네이티브로 수행한다 — 목표 텍스트에 작업 키워드
(뽑아줘/목록/파일로/다운로드/추출/정리/요약/extract/download/list 등)가 있으면 `run` 이
자동으로 라우팅한다(`--task` 강제, `--scan` 스캔 강제).

```bash
redcell run --host axhack.lge.com --goal "하반기 1차 합격자 목록 리스트 파일로 뽑아줘"
# → 페이지를 읽고(fetch_page) 링크를 따라가며 → 첨부를 내려받고(download_file)
#   → 목록을 정리해 파일로 저장(write_output) → 산출물: ~/.redcell/results/<host>-<ts>/
```

- **목표 툴 3종**(`src/tools/goal-tools.ts`): `fetch_page`(본문 텍스트·링크·파일 링크·폼 추출),
  `download_file`(바이너리 원본 보존 저장 — PDF/엑셀/HWP), `write_output`(정리본 파일 작성).
  취약점 주입 툴은 작업 모드에서 제외된다(목표 이탈 방지).
- **목표 지향 루프** — 종료는 ① 모델 선언+추궁, ② 소득 정체, ③ 안전 가드(60액션/15분).
  모델이 파일을 안 만들고 끝내도, 최종 종합이 수집 내용으로 정리본이라도 만들어 낸다.
- **산출물 위치**: `~/.redcell/results/<host>-<타임스탬프>/` — 실행 중 `[산출물] <경로>`
  이벤트로 실시간 표시된다.
- 안전: 모든 요청은 ScopeGuard(recon)를 통과하고, 파일 쓰기는 산출물 디렉터리 안으로만
  제한(경로 조작 차단). 다운로드는 같은 오리진으로만.
