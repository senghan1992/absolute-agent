...
- **서비스 진단 모드(상단 '서비스 진단' 토글)** — 서비스 설명(목적·기술 스택·인증
  방식·주요 기능·데이터 흐름·외부 연동)을 붙여넣고 보내면, 에이전트가 **창의적 공격
  포인트 + 개선·보완 권고**를 리포트 형식으로 작성한다: ① 진단 범위·가정 ② 공격 표면
  ③ 공격 루트(위험도·시나리오·영향) ④ 보완 권고(우선순위 P0~P3·구체적 방법·검증)
  ⑤ URL 을 함께 넣으면 실측으로 가정 검증(web_fetch). 잘 알려진 취약점뿐 아니라
  조합·비즈니스 로직·운영 방식의 루트를 발굴하고, 모든 루트에 "미리 막는 방법"을
  제시한다. 리포트는 결과 탭에서 표·목록으로 렌더링된다.
